...
<template>
    <div>
      ...
      <MyBackendsidebar></MyBackendsidebar>
        ...
        <div class="main">
                 <div class="title" style="width:1000px;float:left">
                   <h3 class="card-title">All Pending Book {{today}}</h3>
                 </div>
      ...
      <tbody>
       ...
       <tr v-for="(v_show,index) in allpendingbook">
...
<script>
   ...
   import Mybackendfooter from "../backendlayout/Adminfooter.vue"
    ...
    methods:{
     ...
     axios.get('/show_all_pending_book').then((res) => {
...
//pendingbooksuccess(v_show.studenttake_libary_book_id)
      ...
      pendingbooksuccess(studenttake_libary_book_id){

      axios.get('/pending_book_success/'+studenttake_libary_book_id)
      .then((response)=>{
        Toast.fire({
        icon: 'success',
        title: 'Book Successsfully Return'
       })
    ...
    //unactive category on
      ...
      successbookpending(studenttake_libary_book_id){

        axios.get('/success_book_pending/'+studenttake_libary_book_id)
        .then((response)=>{

          Toast.fire({
          icon: 'error',
          title: 'Book Not Successsfully Return'
         })
